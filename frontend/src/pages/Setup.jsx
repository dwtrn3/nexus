...
const ALL_STEPS = ['Google Chat', 'WhatsApp', 'Slack', 'Gmail']
const STEPS = ['WhatsApp', 'Slack', 'Gmail'] // default (Google users)
const ICONS = {
  'Google Chat': (
...
function GoogleChatStep({ onDone, onSkip }) {
  const [connected, setConnected] = useState(false)
  const [connecting, setConnecting] = useState(false)
  const [searchParams] = useSearchParams()
  ...
  async function checkStatus() {
    try {
      const res = await api.get('/google-chat/status')
      if (res.data.connection?.status === 'connected') {
        setConnected(true)
  ...
  function handleConnect() {
    setConnecting(true)
    // Google Chat uses same Google OAuth flow as auth
  ...
  if (connected) {
    return (
      <div className="text-center py-6">
        ...
        <p className="font-semibold text-gray-900">Google Chat Connected!</p>
        <p className="text-xs text-gray-400 mt-1">Internal messages are ready</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <p className="text-sm font-medium text-blue-900 mb-1">Internal communications</p>
        <p className="text-sm text-blue-700">Google Chat handles your internal team messages. Connect your Google account to access it.</p>
      </div>
      <button
        onClick={handleConnect}
        disabled={connecting}
...
export default function Setup() {
  const [steps, setSteps] = useState(STEPS) // may add 'Google Chat' prepended
  const [currentStep, setCurrentStep] = useState(0)
  const { user, refreshAuth } = useAuth()
  const navigate = useNavigate()
  const [searchParams] = useSearchParams()

  // Determine if we need to prompt Google Chat (email users only)
  useEffect(() => {
    async function checkGoogleChat() {
      try {
        const res = await api.get('/google-chat/status')
        const hasGoogleChat = res.data.connection?.status === 'connected'
        // If no Google Chat, prepend that step for email-login users
        if (!hasGoogleChat) {
          setSteps(['Google Chat', 'WhatsApp', 'Slack', 'Gmail'])
        }
      } catch {}
    }
    checkGoogleChat()
  ...
  useEffect(() => {
    const offset = steps[0] === 'Google Chat' ? 1 : 0
    if (searchParams.get('slack_success') || searchParams.get('slack_error')) setCurrentStep(1 + offset)
    if (searchParams.get('gmail_success') || searchParams.get('gmail_error')) setCurrentStep(2 + offset)
  }, [steps])
  ...
  const allStepComponents = {
    'Google Chat': <GoogleChatStep key="gchat" onDone={handleStepDone} onSkip={handleSkip} />,
    'WhatsApp': <WhatsAppStep key="wa" onDone={handleStepDone} onSkip={handleSkip} />,
    'Slack': <SlackStep key="slack" onDone={handleStepDone} onSkip={handleSkip} />,
    'Gmail': <GmailStep key="gmail" onDone={handleStepDone} onSkip={handleSkip} />
  }
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="card p-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: channelColors[currentStepName] }}>
              {ICONS[currentStepName]}
            </div>
            <div>
              <h2 className="font-semibold text-gray-900">Connect {currentStepName}</h2>
              <p className="text-xs text-gray-400">Step {currentStep + 1} of {steps.length}</p>
            </div>
          </div>

          {allStepComponents[currentStepName]}
        </div>
